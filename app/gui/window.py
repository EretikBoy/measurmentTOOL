...
from PyQt6.QtCore import Qt, QEvent, QThread, pyqtSignal, pyqtSlot
import os
import shutil
import pickle
import pandas as pd
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Группа управления приборами
        instruments_group = QGroupBox("Управление приборами")
        instruments_layout = QVBoxLayout()
        
        # Выбор приборов
        instruments_select_layout = QHBoxLayout()
        
        instruments_select_layout.addWidget(QLabel("Генератор (Rigol):"))
        self.rigol_combo = QComboBox()
        instruments_select_layout.addWidget(self.rigol_combo)
        
        instruments_select_layout.addWidget(QLabel("Осциллограф (GWInstek):"))
        self.gwinstek_combo = QComboBox()
        instruments_select_layout.addWidget(self.gwinstek_combo)
        
        self.refresh_instruments_button = QPushButton("Обновить список")
        self.refresh_instruments_button.clicked.connect(self.refresh_instruments_list)
        instruments_select_layout.addWidget(self.refresh_instruments_button)
        
        instruments_layout.addLayout(instruments_select_layout)
        
        # Параметры измерения
        measurement_params_layout = QHBoxLayout()
        
        measurement_params_layout.addWidget(QLabel("Нач. частота:"))
        self.start_freq_edit = QLineEdit("100")
        measurement_params_layout.addWidget(self.start_freq_edit)
        
        measurement_params_layout.addWidget(QLabel("Конеч. частота:"))
        self.end_freq_edit = QLineEdit("1000")
        measurement_params_layout.addWidget(self.end_freq_edit)
        
        measurement_params_layout.addWidget(QLabel("Время записи:"))
        self.record_time_edit = QLineEdit("10")
        measurement_params_layout.addWidget(self.record_time_edit)
        
        instruments_layout.addLayout(measurement_params_layout)
        
        # Кнопки управления
        instruments_buttons_layout = QHBoxLayout()
        
        self.measure_button = QPushButton("Начать запись")
        self.measure_button.clicked.connect(self.start_measurement)
        self.set_button_style(self.measure_button, 'normal')
        instruments_buttons_layout.addWidget(self.measure_button)
        
        self.stop_button = QPushButton("Остановить")
        self.stop_button.clicked.connect(self.stop_measurement)
        self.stop_button.setEnabled(False)
        instruments_buttons_layout.addWidget(self.stop_button)
        
        instruments_layout.addLayout(instruments_buttons_layout)
        ...
        # Обновляем список приборов при запуске
    ...
    def refresh_instruments_list(self):
        """Обновление списка доступных приборов"""
        self.rigol_combo.clear()
        self.gwinstek_combo.clear()
        
        # Здесь должен быть код для обнаружения доступных приборов
        # Для примера добавляем заглушки
        self.rigol_combo.addItem("USB0::0x1AB1::0x0588::DG1D140300224::INSTR")
        self.gwinstek_combo.addItem("COM8")
        
        self.log_text.append("Списки приборов обновлены")
    ...
    def start_measurement(self):
        ...
        try:
            ...
            record_time = float(self.record_time_edit.text())
        ...
        rigol_resource = self.rigol_combo.currentText()
        gwinstek_port = self.gwinstek_combo.currentText()
        
        if not rigol_resource or not gwinstek_port:
            self.log_text.append("Выберите оба прибора")
            return
        ...
        self.measurement_thread = InstrumentWorker(rigol_resource, gwinstek_port, params)
    ...
    def measurement_finished(self, channels_data):
        ...
        self.stop_button.setEnabled(False)
        self.set_button_style(self.measure_button, 'success')
    ...
    def measurement_error(self, error_message):
        ...
        self.measure_button.setEnabled(True)
        self.stop_button.setEnabled(False)
        self.set_button_style(self.measure_button, 'error')
